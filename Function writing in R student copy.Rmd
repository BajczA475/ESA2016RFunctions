...
# OVERVIEW
This course is designed to teach the essential skills necessary to build custom functions in the R software platform. Custom functions can automate a lot of work that would otherwise have to be done by hand over and over again. Moreover, functions can be used to accomplish some pretty amazing feats, including simulating real-world outcomes, running thousands of computations in seconds, and producing publishable-quality graphs with little additional user input.  

# LEARNING OBJECTIVES
* Converse meaningfully in the programming terminology of R.
* Use functions, indexing, and other programming tools to usefully manipulate objects.
* Apply multiple approaches to troubleshooting faulty code as well as building better code.
* Recognize situations where work could be done faster and better using a custom function.
* Construct custom functions using pre-existing R functions and their parts, R help pages, and the internet.
...
This **matrix** has 2 rows and 5 columns. There is another common type of **object** called a *data frame* that looks a lot like a **matrix**. *Data frames* are useful when you have a lot of different kinds of data, while **matrices** are better for working with just numbers. Today's class will work just with **matrices**.  

You can also make **objects** with three or more dimensions. These are called *arrays*. For example, a 3D array can be thought of as "sheets" of matrices stacked directly on top of one another. These behave a lot like **matrices**, but they're a little hard to think about, so we won't use them today either.  
...
We're telling R: "Go look in `box1` and find the 8th entry and tell me what it is." It's like every entry in our box has a specially numbered cubby just for it!  
...
To use a **function**, we write what's called a **function call**. It's like the **function** is a buddy we're calling up and asking to do some work for us. A **call** has three parts--the **name** of the **function**, parentheses after that **name**, and then input inside those parentheses we want the **function** to operate on. Consider the **object** `useful.numbers`. We can create this **object** using the `c` function.

```{r eval = FALSE}
useful.numbers = c(50,70)
```

The right side of the above line is a **call** to the `c` **function**, and the numbers we want it to combine are put inside the parentheses and seperated by commas. We then can **call** other **functions** to learn more about the values in this **object**.  
...
```
Which of the following lines of code would return an error?  
A. `mean(0)`  
B. `mean(vector2, 0)`  
C. `mean(vector2[1:5})`  
...
`==` is an example of "logic punctuation." These are the symbols you need to write your **logical expressions**. We have greater than and less than and their related forms:  
...
Maybe this isn't what we want `sample` to do. Imagine you're running a simulation model using a `for` loop. Each time through the loop in the simulation, a **vector** gets created that stores the ID numbers of all the turtles who ate any radioactive bugs that time through the loop. Each time step, you want to use `sample` to randomly select 1 turtle from that **vector** so that turtle becomes a Ninja turtle. What if, a few times, only one turtle actually ate a radioactive bug, so the **vector** looked like this?  
...
One way to catch them is the double-click trick. If you double-click or highlight an **object name** in R Studio, it'll also highlight all the other times that **object name** is in your code. If **object names** that should match don't all highlight, chances are you have a typo in one of them.  
...
words = function(length=12, num.letters=3) { 
  ...
  answer[choices] = spots 
  return(answer)
}
```

When the **function** works correctly, it should produce a result like this:  
...
Repeat this process for the other operations.  
...
We suggest that you work with as many people as possible--coding is best when it's done collaboratively, and the more brains, the better.